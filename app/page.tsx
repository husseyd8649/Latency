...
export default async function Landing() {
  ...
  const comparisonFeatures = [
    { name: "HTTP monitoring", us: true, them: true },
    { name: "TCP port checks", us: true, them: false },
    { name: "SSL expiry monitoring", us: true, them: false },
    { name: "Public status pages", us: true, them: true },
    { name: "HMAC-signed webhooks", us: true, them: false },
    { name: "Bulk CSV import", us: true, them: false },
    { name: "Zero-config setup", us: true, them: false },
    { name: "Regional grouping", us: true, them: false },
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {/* Comparison - Revamped Bento Style */}
      <section id="comparison" className="border-b border-[var(--border)] relative overflow-hidden">
        <div
          className="absolute inset-0 opacity-[0.03] pointer-events-none"
          style={{
            background: `radial-gradient(ellipse 80% 50% at 50% 50%, #2563EB, transparent 70%)`,
          }}
        />
        ...
        <div className="max-w-6xl mx-auto px-6 py-20 relative">
          <div className="max-w-2xl mb-12">
            ...
            <p className="text-[var(--text-muted)] mt-3 text-base leading-relaxed">
              Feature-rich without the bloat. See how we compare to generic uptime monitors.
            </p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Latency Column */}
            <div className="rounded-2xl border border-[var(--accent)]/20 bg-[var(--surface)] p-1 shadow-[var(--shadow-md)] relative overflow-hidden">
              <div
              ...
              <div className="relative p-6">
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#2DD4BF] to-[#2563EB] flex items-center justify-center shadow-lg">
                    <Zap className="w-5 h-5 text-white" strokeWidth={2.5} />
                  </div>
                  <div>
                    <h3 className="text-lg font-bold">Latency</h3>
                    <p className="text-xs text-[var(--text-muted)]">Built for shipping teams</p>
                  </div>
                </div>
                
                <ul className="space-y-3">
                  {comparisonFeatures.map((feature) => (
                    <li key={feature.name} className="flex items-center gap-3 p-3 rounded-lg bg-[var(--surface-2)]/50 border border-[var(--border)]">
                      <div className="w-6 h-6 rounded-full bg-[var(--accent-soft)] flex items-center justify-center shrink-0">
                        <Check className="w-3.5 h-3.5 text-[var(--accent)]" strokeWidth={3} />
                      </div>
                      <span className="text-sm font-medium text-[var(--text)]">{feature.name}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {/* Others Column */}
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)]/50 p-1 opacity-80">
              <div className="p-6">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-[var(--text-muted)]">Generic tools</h3>
                    <p className="text-xs text-[var(--text-muted)]">Basic uptime checking</p>
                  </div>
                </div>
                  ...
                  {comparisonFeatures.map((feature) => (
                    <li 
                      ...
                      className={`flex items-center gap-3 p-3 rounded-lg border ${
                        feature.them 
                          ? "bg-[var(--surface-2)]/30 border-[var(--border)]" 
                          : "bg-[var(--surface-2)]/10 border-[var(--border)]/50 opacity-50"
                      }`}
                    >
                      {feature.them ? (
                        <div className="w-6 h-6 rounded-full bg-[var(--surface-2)] flex items-center justify-center shrink-0">
                          <Check className="w-3.5 h-3.5 text-[var(--text-subtle)]" strokeWidth={3} />
                        </div>
                      ) : (
                        <div className="w-6 h-6 rounded-full bg-[var(--down-soft)] flex items-center justify-center shrink-0">
                          <X className="w-3.5 h-3.5 text-[var(--op-down)]" strokeWidth={3} />
                        </div>
                      )}
                      <span className={`text-sm ${feature.them ? "text-[var(--text)]" : "text-[var(--text-muted)] line-through"}`}>
                        {feature.name}
                      </span>
                    </li>
                  ))}
          ...
          <p className="text-[11px] text-[var(--text-subtle)] mt-6 text-center">
